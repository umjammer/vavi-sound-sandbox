...
/**
 * Base class for types of audio filter/converter that translate one frame to
 * another frame.<br>
...
Debug.println("TSynchronousFilteredAudioInputStream: original format =" + AudioUtils.format2ShortStr(originalFormat));
Debug.println("TSynchronousFilteredAudioInputStream: converted format=" + AudioUtils.format2ShortStr(getFormat()));
...
//        }
     ...
     * This method must always convert frameCount frames, so no return value is
     * necessary.
     */
...
Debug.println("> TSynchronousFilteredAIS.read(buffer[" + abData.length + "], " + nOffset + " ," + nLength + " bytes ^="
       + nFrameLength + " frames)");
...
Debug.println("original.read returned " + nBytesRead + " bytes ^=" + nFramesRead + " frames");
        if (m_bConvertInPlace) {
...
Debug.println("< converted " + nFramesConverted + " frames");
