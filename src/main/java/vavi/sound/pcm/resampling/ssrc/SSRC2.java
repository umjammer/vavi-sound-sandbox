...
//  private static final int M = 15;
     ...
     * @throws IOException 
     */
    double upsample(FileChannel fpi, FileChannel fpo, int nch, int bps, int dbps, int sfrq, int dfrq, double gain, int chanklen, boolean twopass, int dither) throws IOException {
...
//          delta = Math.pow(10, -aa / 20);
...

        {
            ...
            while (true) {
                ...
                ending = fpi.position() == fpi.size() || sumread >= chanklen;
...
//System.err.printf("%d : %g %g %g %g %g %g\n",i,stage2[i*2],stage2[i*2+1],buf2[ch][i*2],buf2[ch][i*2+1],re,im);
    ...
    /** */
    double downsample(FileChannel fpi, FileChannel fpo, int nch, int bps, int dbps, int sfrq, int dfrq, double gain, int chanklen, boolean twopass, int dither) throws IOException {
        int frqgcd, osf = 0, fs1, fs2;
        double[] stage1;
        double[][] stage2;
...
//System.err.print(StringUtil.getDump(rawoutbuf, i, 8));
    ...
    /** */
    double no_src(FileChannel fpi, FileChannel fpo, int nch, int bps, int dbps, double gain, int chanklen, boolean twopass, int dither) throws IOException {
    ...
    /** */
    SSRC2(String[] argv) throws IOException {
